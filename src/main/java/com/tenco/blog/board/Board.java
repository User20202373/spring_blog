...
public class Board {
    ...
    private Integer id;
    // private String username; 삭제해야함
    private String title;
    private String content;

    //연관관계 설정 해주어야 한다
    // 1 : 1'
    // Board, User --> 1 : 1, 1 : N, N : 1
    //  @ManyToOne-> N:1 다대일 연관관계 : 여러개 게시글이 하나의 사용자에게 속한다
    //.fetchType 전략 : EAGER, LAZY
    //EAGER - 조희시 한 번에 다 들고와라 ( 1번 게시글 조회시 한 번 조인까지 해라)
    ...
    public void update(BoardRequest.UpdateDTO updateDTO) {
        ...
        //2. 영속화된 엔티티가(board)의 멤버 변수 값이 변경이 된다면
        // 1차에서 보관했던 값과 2차에서 수정된 필드값을 비교 함.
        //3. 변화가 발생이 되었다면 트랜잭션 커밋 시점에 변경된 필드만 UPDATE쿼리 자동 생성
        //4. 물리적인 DB에 반영 됨.
    }
